...
<script type="text/javascript">
		...
		function query(){
			...
			$('#tab').datagrid({
				...
				toolbar:'#tb',
				height:400,
				url:'gets/rechargeLog',
				loadMsg:'正在查询...',
				onLoadSuccess:function(data){
				    if(data.total == 0){
				    	$('#info').html('无充值记录');
				    }else{
				    	$('#info').html('共找到' + data.total + '个充值记录');
				    }
				},
				...
				columns:[[
					...
					{field:'surplus',title:'剩余金额',width:25,sortable:true},
		...
		function expExcel(){
			window.location.href=encodeURI(encodeURI("export/rechargeLog?player=" + param.player + "&minTime=" + param.minTime + "&maxTime=" + param.maxTime));
		}
	</script>
</head>
