 ...
 * 
 * @author HuangDeCai
 * @since 2012-12-30 9:30:57
 */
@Transactional
@Service
public class MobilityLogServiceImpl implements MobilityLogService
{
	private Logger log = LoggerFactory.getLogger(this.getClass());
	@Resource
	private MobilityLogDao mobilityLogDao;
	@Resource
			...
			String order, Integer player, String startTime, String endTime)
	{
		...
		result.put("rows", new ArrayList<MobilityLog>());
		result.put("total", 0);
		...
		try
		{
			...
			if (startTime != null && endTime != null && !"".equals(startTime)
					&& !"".equals(endTime))
			{
				total = mobilityLogDao.count(player, startTime, endTime);
				list = mobilityLogDao.list(rows, page, sort, order, player,
						startTime, endTime);

			} else
			{
				total = mobilityLogDao.count(player);
				list = mobilityLogDao.list(rows, page, sort, order, player);
			}
	...
	public List<MobilityLog> gets(String nickname, String startTime,
			String endTime)
	{
		List<MobilityLog> result = new ArrayList<MobilityLog>();
		Integer player = null;
		if (nickname != null)
		{
			nickname = nickname.trim();
	...
	public List<MobilityLog> gets(Integer player, String startTime,
			String endTime)
	{
		List<MobilityLog> result = new ArrayList<MobilityLog>();
		try
		{
			if (startTime != null && endTime != null && !"".equals(startTime)
					&& !"".equals(endTime))
			{
				result = mobilityLogDao.list(player, startTime, endTime);
			} else
			{
				result = mobilityLogDao.list(player);
			}
		} catch (Exception e)
		{
			log.error(e.getMessage());
