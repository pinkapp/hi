package cc.ywxm.action.platform;

import java.io.IOException;
import java.io.OutputStream;
import java.io.PrintWriter;
import java.util.List;

import javax.annotation.Resource;

import org.springframework.context.annotation.Scope;
import org.springframework.stereotype.Controller;

import cc.ywxm.action.PageableAction;
import cc.ywxm.constant.Const;
import cc.ywxm.datasource.CustomerContextHolder;
import cc.ywxm.excel.ExportExcel;
import cc.ywxm.model.gm.Server;
import cc.ywxm.model.platform.CareerCount;
import cc.ywxm.service.platform.CareerCountService;
import cc.ywxm.utils.DateFormat;
import cc.ywxm.utils.JSONtool;

/**
 * 玩家职业统计
 * 
 * @author HuangDeCai
 * @since 2012-12-20 13:18:17
 * 
 */
@SuppressWarnings("serial")
@Controller
@Scope("prototype")
public class CareerCountAction extends PageableAction
{

	@Resource
	private CareerCountService careerCountService;

	@Override
	public String execute() throws Exception
	{
		return super.execute();
	}

	/**
	 * 查询
	 * 
	 * @return
	 * @throws IOException
	 */
	public String gets() throws IOException
	{
		response.setContentType("text/xml;charset=utf-8");
		response.setHeader("Cache-Control", "no-cache");
		PrintWriter out = response.getWriter();
		String jsonString = JSONtool.serialize(careerCountService.gets());
		out.append(jsonString);
		out.close();

		return NONE;
	}

	/**
	 * 图表
	 * 
	 * @return
	 * @throws IOException
	 */
	public String chart() throws IOException
	{
		Server dbsrv = (Server) session.get(Const.SESSION_DBSRV);
		CustomerContextHolder.setCustomerType(dbsrv.getDataSource());
		String result = careerCountService.XMLData();
		response.setContentType("text/xml;charset=utf-8");
		response.setHeader("Cache-Control", "no-cache");
		PrintWriter out = response.getWriter();
		out.append(JSONtool.serialize(result));
		out.close();
		return NONE;
	}

	/**
	 * 导出
	 * 
	 * @return
	 * @throws IOException
	 */
	public String export() throws IOException
	{
		Server dbsrv = (Server) session.get(Const.SESSION_DBSRV);
		CustomerContextHolder.setCustomerType(dbsrv.getDataSource());
		List<CareerCount> list = careerCountService.gets();
		ExportExcel<CareerCount> exporter = new ExportExcel<CareerCount>();
		OutputStream out = null;
		response.setContentType("octets/stream");
		response.addHeader("Content-Disposition", "attachment;filename="
				+ DateFormat.currentDateFormat("yyyyMMdd") + ".xls");
		out = response.getOutputStream();
		exporter.exportExcel("玩家职业统计", new String[]
		{ "职业", "人数" }, list, out, "yyyy-MM-dd");
		out.close();
		return NONE;

	}

}
