...
<script type="text/javascript">
		...
		function query(){
			...
			$('#tab').datagrid({
				...
				toolbar:'#tb',
				height:400,
				url:'gets/mobilityLog',
				loadMsg:'正在查询...',
				onLoadSuccess:function(data){
				    if(data.total == 0){
				    	$('#info').html('无使用记录');
				    }else{
				    	$('#info').html('共找到' + data.total + '个使用记录');
				    }
				},
				...
				columns:[[
					...
					{field:'reduce',title:'减少值',width:10,sortable:true},
					{field:'surplus',title:'剩余值',width:25,sortable:true},
		...
		function expExcel(){
			window.location.href=encodeURI(encodeURI("export/mobilityLog?type=" + param.type + "&player=" + param.player + "&minTime=" + param.minTime + "&maxTime=" + param.maxTime));
		}
	</script>
</head>
